		...
		try:
			...
			if beatmapInfo is not None and beatmapInfo != False and s.passed:
				...
				if newScoreboard.personalBestRank == 1 and s.completed == 3 and not restricted:
					...
					# Then post them!
					webhook = Webhook(url, color=0xadd8e6, footer="This score was submitted on Debiki")
					webhook.set_author(name=username.encode().decode("ASCII", "ignore"), icon='https://a.aipserver.ru/{}'.format(userID))
					webhook.set_title(title="New score by {username}!")
